...
function buildTermMatchers(terms: readonly string[]): Map<string, RegExp | null> {
...
function termMatchesLine(line: string, term: string, matcher: RegExp | null): boolean {
  if (matcher === null) {
...
function matchingTermsOnLine(
  line: string,
  terms: readonly string[],
  matchers: ReadonlyMap<string, RegExp | null>,
): string[] {
 ...
 */
function termMatchesHaystack(haystack: string, lower: string, matcher: RegExp | null): boolean {
  if (matcher === null) {
...
function buildHaystackMatchers(terms: readonly string[]): Map<string, RegExp | null> {
  const matchers = new Map<string, RegExp | null>();
  for (const term of terms) {
    const lower = term.toLowerCase();
    matchers.set(
      lower,
      lower.includes(" ") ? null : new RegExp(`(?<![\\w.])${escapeRegex(lower)}(?!\\w)`, "i"),
    );
  }
  return matchers;
}
...
function matchTermsAgainstHaystack(
  haystack: string,
  terms: readonly string[],
  haystackMatchers: ReadonlyMap<string, RegExp | null>,
): Set<string> {
 ...
 */
function stripTimestampPrefix(stem: string): string {
