...
**This grades your machine's installed Grounder runtime and skill files, not this checkout.** `{{GROUNDER_CLI}}` resolves to `~/.grounder/runtime/dist/cli.js`, and the skills a probe actually invokes live under `~/.cursor`/`~/.claude`. Both commands refuse to run unless the runtime exists *and* `grounder status` — read through this checkout's own `packages/grounder/dist/cli.js`, not the installed runtime, since that binary might be a symlink to a different checkout, a global install, or a stale npx cache copy — reports `installCurrent: true`. Run `pnpm build` (checkout not built) and/or `grounder migrate` (install behind this branch) to clear it — otherwise a green sweep could just mean yesterday's install, not this branch's skill-prompt changes.

## How it works

1. **Sandbox** (`lib/sandbox.mjs`) — writes a scratch `~/.grounder`-shaped config under your OS temp dir (`$TMPDIR/grounder-eval/`, **not** inside this repo — see below) pointing `GROUNDER_HOME` at a sandboxed vault, plus a `.grounder.json` project marker in a scratch repo dir unique to each `(model, probe)` run (session-file writes under `--workspace` must not collide across concurrent processes). Search copies the committed `fixtures/eval-vault/` into one shared disposable copy per sweep and locks it read-only (`chmod -R a-w`) — every concurrent probe reads that same copy, so the lock is what stops one probe's write (or delete) attempt from changing what a sibling grades against. Mode-lock gives every `(model, probe)` run its own fully disposable, writable vault (handoff probes write to it for real) seeded with two handoffs (so both a bare load and a `#2` selector probe have something real to resolve) — a *shared* writable vault would let a sibling's write move which file is "#2" out from under a concurrent selector probe.
...
4. **Grade** — search: does the final answer's `## Read these` #1 link match the probe's expected file (`probes/search-probes.json`)? Mode-lock's write-side boundary (recall must never add a file to `logs/`; handoff-control must) is graded off the sandbox vault's own file listing before/after the run — ground truth, independent of *how* a write happened, since a command-text or tool-call check can't see a Bash redirect or a tracked-tool gap that a raw file diff catches for free. Mode-lock's read-side boundary (handoff must never list/peek an existing handoff) has no file-state signature, so that one is still a command-text match — anchored to the actual CLI invocation shape (`probes/mode-lock-probes.json`), not a bare word, so it can't be tripped by an unrelated path that happens to contain "handoff". Either probe set also checks the one-sentence mismatch notice when expected.
...
**This is not a security sandbox.** `--force`/`--allowedTools` control what gets auto-approved, not what's reachable — every probe runs with your real `$HOME`, so a model's Bash can read or write `~/.grounder`, `~/.cursor`, `~/.claude`, and your real vault if it discovers the path (nothing here stops it, and a probe that goes looking has no reason to unless it's actively adversarial about it). Acceptable for a maintainer-only tool run by whoever already has full access to that machine; not something to point at a model or workspace you don't already trust.
...
In practice this is a smaller compromise than it sounds: since each probe runs in its own scratch cwd (`$TMPDIR/grounder-eval/search-repo/<key>/`, `$TMPDIR/grounder-eval/mode-lock/<key>/repo/` — `<key>` a hash unique to that `(model, probe)` run), the host CLI's per-project session transcripts land under a distinctly-named entry in your real `~/.claude/projects/` / `~/.cursor/projects/` — separate from, and never mixed into, this repo's own conversation history.
