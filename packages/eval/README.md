...
1. **Sandbox** (`lib/sandbox.mjs`) — writes a scratch `~/.grounder`-shaped config under your OS temp dir (`$TMPDIR/grounder-eval/`, **not** inside this repo — see below) pointing `GROUNDER_HOME` at a sandboxed vault, plus a `.grounder.json` project marker in a scratch repo dir. Search copies the committed `fixtures/eval-vault/` into a disposable scratch copy every run and locks it read-only (`chmod -R a-w`) — every concurrent probe shares that one copy, so without the lock a probe that writes (or deletes) could change what a sibling probe grades against. Mode-lock wipes and recreates a fully disposable, writable vault every run (handoff probes write to it for real) and seeds two handoffs (so both a bare load and a `#2` selector probe have something real to resolve).
